...
function subject_callback($vals, $style, $output_mod) {
    ...
    return '<td class="subject"><div class="'.$output_mod->html_safe(implode(' ', $vals[2])).'"><a title="'.$output_mod->html_safe(implode(' ', $vals[2])).'" href="'.$output_mod->html_safe($vals[1]).'">'.$subject.'</a><p class="fw-light" title="'. $preview_msg .'">'. $preview_msg .'</p></div></td>';
}}
